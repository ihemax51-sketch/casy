...
static bool placeHook(int trampoline_location, int target_location)
{
	...
	FlushInstructionCache(GetCurrentProcess(), (LPVOID)trampoline_location, sizeof(jmp_inst));
	DWORD ignoredProtect = 0;
	return VirtualProtect((LPVOID)trampoline_location, sizeof(jmp_inst), dwProtect, &ignoredProtect) != FALSE;
}
...
static bool replaceOffset(int trampoline_location, int target_location)
{
	...
	FlushInstructionCache(GetCurrentProcess(), (LPVOID)offset_location, sizeof(inst_offset));
	DWORD ignoredProtect = 0;
	return VirtualProtect((LPVOID)offset_location, sizeof(inst_offset), dwProtect, &ignoredProtect) != FALSE;
}

static bool replaceAddr(int addr, int value)
{
	DWORD dwProtect;
	...
	*((int*)addr) = value;
	FlushInstructionCache(GetCurrentProcess(), (LPVOID)addr, sizeof(int));
	DWORD ignoredProtect = 0;
	return VirtualProtect((LPVOID)addr, sizeof(int), dwProtect, &ignoredProtect) != FALSE;
}
