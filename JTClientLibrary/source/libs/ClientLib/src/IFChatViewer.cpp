...
bool CIFChatViewer::OnCreate(long ln) {
    ...
    for (int i = 0; i < numberOfTabs; i++) {
        ...
        if (i) {
            switch (i) {
                ...
                case 4:
                    m_pTabs[4]->SetText(KmtGetText(L"UIIT_KMT_MARKET"));
                    break;
            }
    ...
    UpdateChatSize();
    std::n_wstring EmojiList = KmtGetText(L"UIIT_KMT_EMOJI_LIST");
        ...
        case VK_RETURN: {
            ...
            switch (input_text[0]) {
                ...
                case '%':
                    ...
                    break;
                case '~':
                    N00009C5E = CHAT_Stall;
                    break;
                default:
                    if (m_currentTabPage == TAB_MARKET) {
                        N00009C5E = CHAT_Stall;
                    } else if (g_pMyPlayerObj->IsGameMaster()) {
            ...
            if (!sub_4F9C50()) {
                ...
                if (N00009C5E == CHAT_PM) {
                    ...
                    if (N00009C5E == 254) {
                        ...
                        if (N00009C5E != CHAT_All && N00009C5E != CHAT_AllGM && N00009C5E != CHAT_Global) {
                            const bool hasMarketPrefix = N00009C5E == CHAT_Stall && input_text[0] == '~';
                            const bool shouldStripPrefix = N00009C5E != CHAT_Stall || hasMarketPrefix;
                            if (shouldStripPrefix)
                                pMessage++;

                            if (shouldStripPrefix && input_text.length() == 1) {
                                Unfocus();
                                return 1;
                            }
                ...
                if (N00009C5E == CHAT_AllGM) {
                    ...
                    m_InputBox->SetText(L"%");
                } else if (m_currentTabPage == TAB_MARKET || (N00009C5E == CHAT_Stall && m_currentTabPage == TAB_ALL)) {
                    m_InputBox->SetText(L"~");
                } else {
                    m_InputBox->SetText(L"");
#if 0
...
void CIFChatViewer::ActivateTabPage(eTabPage page) {
    ...
    switch (page) {
        ...
        case TAB_MARKET:
            ...
            if (m_chatSize)
                m_ChatLists[TAB_MARKET]->ShowGWnd(true);
...
void CIFChatViewer::FUN_007aca30(ChatType chatType, D3DCOLOR color, const wchar_t *message, int param4, int param5) {
    ...
    if (chatType == CHAT_Stall) {
        if (m_ChatLists[TAB_MARKET]) {
            std::n_wstring strmsg(message);
            if (m_numberOfLinesPerTab[TAB_MARKET]++ > 250)
                m_numberOfLinesPerTab[TAB_MARKET] = 1;

            m_ChatLists[TAB_MARKET]->sub_64F8A0(strmsg, m_numberOfLinesPerTab[TAB_MARKET], color, color, -1, 0, param5);
        }
        return;
    }
