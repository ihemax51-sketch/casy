...
public:
    ...
    void SetTargetObject(int objectId);
    void SetTargetName(std::n_wstring* name, int unk);
    void RefreshHpPercentText();
    bool RefreshHpPercentTextForObjectHp(unsigned int objectId, unsigned int currentHp);
    void OnClickPossibleDrops();

private:
    int m_objectId; // 0x36C
...
private:
    BEGIN_FIXTURE()
        ENSURE_SIZE(0x37c)
        ENSURE_OFFSET(m_objectId, 0x36C)
