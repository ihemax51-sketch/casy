/*
    Adds restart-applied Party Monster spawning controls for the GameServer.

    Defaults preserve the original vSRO 188 behavior:
      EnablePartyMonsterSpawn    = True
      PartyMonsterMinimumMembers = 2
      PartyMonsterSpawnRate      = 50
*/
...
(
    ...
    THROW 51001, 'Duplicate Party Monster settings already exist in dbo.System_Settings.', 1;
GO
...
BEGIN
    ...
    VALUES (N'EnablePartyMonsterSpawn', N'True');
END;
...
BEGIN
    INSERT dbo.System_Settings (SettingName, Value)
    VALUES (N'PartyMonsterMinimumMembers', N'2');
END;
...
BEGIN
    INSERT dbo.System_Settings (SettingName, Value)
    VALUES (N'PartyMonsterSpawnRate', N'50');
END;
...
BEGIN
    EXEC(N'
        ...
        SET Category = N''GameServer.Monsters'',
            DisplayOrder = CASE SettingName
                WHEN N''EnablePartyMonsterSpawn'' THEN 10
                WHEN N''PartyMonsterMinimumMembers'' THEN 20
                WHEN N''PartyMonsterSpawnRate'' THEN 30
                ELSE DisplayOrder
            END,
            Description = CASE SettingName
                WHEN N''EnablePartyMonsterSpawn'' THEN N''Enable or disable Party Monster spawning. Requires a GameServer restart.''
                WHEN N''PartyMonsterMinimumMembers'' THEN N''Minimum party-member count required for Party Monster spawning, from 1 to 9. Requires a GameServer restart.''
                WHEN N''PartyMonsterSpawnRate'' THEN N''Party Monster spawning chance from 0 to 100 percent. Requires a GameServer restart.''
                ELSE Description
            END
        WHERE SettingName IN
        (
...
PRINT 'Party Monster GameServer settings are available with original vSRO defaults.';
GO
