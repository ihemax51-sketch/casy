/*
    Adds the customer-controlled DisableDurability GameServer setting.

    False = normal durability loss.
    True  = prevent durability from decreasing after the GameServer restarts.
*/
...
(
    ...
    WHERE SettingName = N'DisableDurability'
)
BEGIN
    INSERT dbo.System_Settings (SettingName, Value)
    VALUES (N'DisableDurability', N'False');
END;
GO
...
BEGIN
    EXEC(N'
        ...
        SET Category = N''Security.Limits'',
            DisplayOrder = 90,
            Description = N''Prevents weapon and equipment durability from decreasing. Requires a GameServer restart.''
        WHERE SettingName = N''DisableDurability'';
    ');
END;
GO
...
PRINT 'DisableDurability is available and defaults to False.';
GO
