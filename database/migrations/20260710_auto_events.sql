...
-- Superseded by database/migrations/20260711_events_database.sql.
-- The runtime Auto Events service now reads/writes Events.dbo._AutoEvent* tables.
...
IF COL_LENGTH(N'dbo._AutoEventConfig', N'StartDelaySeconds') IS NULL
    ALTER TABLE dbo._AutoEventConfig ADD StartDelaySeconds int NOT NULL CONSTRAINT DF_AutoEventConfig_StartDelay DEFAULT (60) WITH VALUES;
GO
...
IF NOT EXISTS (
    ...
    WHERE name = N'CK_AutoEventConfig_StartDelay'
      AND parent_object_id = OBJECT_ID(N'dbo._AutoEventConfig')
)
    ALTER TABLE dbo._AutoEventConfig WITH CHECK ADD CONSTRAINT CK_AutoEventConfig_StartDelay CHECK (StartDelaySeconds BETWEEN 0 AND 3600);
GO
...
WHEN NOT MATCHED THEN
    INSERT (EventCode, DisplayName, Enabled, StartDelaySeconds, RoundCount, RoundDurationSeconds, InterRoundDelaySeconds, MinLevel,
            HwidLimit, UniqueWinnerPerRun, RequireHwid, AnswerCooldownMs, AlchemyTargetPlus)
...
PRINT 'Auto Events installed. Use EXEC dbo._AutoEventEnqueueStart N''Retype'' from _Scheduler, or console: /event start Retype.';
GO
