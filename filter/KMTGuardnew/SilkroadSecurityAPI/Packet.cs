...
// The Silkroad wire format in this library relies on the legacy UTF-7 transform.
...
namespace SilkroadSecurityAPI
{
    public class Packet
    {
        ...
        private PacketWriter m_writer = null!;
        ...
        public String ReadAscii(int codepage)
        {
            ...
            var encoding = CodePagesEncodingProvider.Instance.GetEncoding(codepage)
                ?? throw new NotSupportedException($"Code page {codepage} is not available.");
        ...
        public void WriteAscii(String value, int code_page)
        {
            ...
            var encoding = CodePagesEncodingProvider.Instance.GetEncoding(code_page)
                ?? throw new NotSupportedException($"Code page {code_page} is not available.");
        ...
        public void WriteAscii(object value, int code_page)
        {
            ...
            var encoding = CodePagesEncodingProvider.Instance.GetEncoding(code_page)
                ?? throw new NotSupportedException($"Code page {code_page} is not available.");
            byte[] codepage_bytes = encoding.GetBytes(text);
            string utf7_value = Encoding.UTF7.GetString(codepage_bytes);
            byte[] bytes = Encoding.Default.GetBytes(utf7_value);
        ...
        public void WriteUnicode(object value)
        {
            ...
            m_writer.Write((ushort)text.Length);
            m_writer.Write(bytes);
        }
