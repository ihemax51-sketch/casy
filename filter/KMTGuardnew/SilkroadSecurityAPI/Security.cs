...
namespace SilkroadSecurityAPI
{
    public class Security
    {
        ...
        private TransferBuffer _mCurrentBuffer = null!;
        ...
        private int _mMassiveBytes;
        private Packet _mMassivePacket = null!;
        private readonly int _maxMassiveFragments;
        private readonly int _maxMassiveBytes;
        private static long _rejectedMassivePackets;
        public static long RejectedMassivePackets =>
            Interlocked.Read(ref _rejectedMassivePackets);
        ...
        // Default constructor
        public Security(int maxMassiveFragments = 1024, int maxMassiveBytes = 16 * 1024 * 1024)
        {
            if (maxMassiveFragments <= 0 || maxMassiveFragments > ushort.MaxValue)
                throw new ArgumentOutOfRangeException(nameof(maxMassiveFragments));
            if (maxMassiveBytes <= 0)
                throw new ArgumentOutOfRangeException(nameof(maxMassiveBytes));

            _maxMassiveFragments = maxMassiveFragments;
            _maxMassiveBytes = maxMassiveBytes;
            _mValueX = 0;
            _mValueG = 0;
            _mValueP = 0;
            _mValueA = 0;
                            ...
                            if (packet_opcode == 0x600D) // Auto process massive messages for the user
                            {
                                ...
                                if (mode == 1)
                                {
                                    if (_mMassivePacket != null)
                                        ThrowMalformedMassive("A new massive packet started before the previous packet completed.");

                                    var fragmentCount = packet_data.ReadUInt16();
                                    if (fragmentCount == 0 || fragmentCount > _maxMassiveFragments)
                                        ThrowMalformedMassive("The massive packet fragment count is outside the configured limit.");

                                    _mMassiveCount = fragmentCount;
                                    _mMassiveBytes = 0;
                                ...
                                {
                                    var massivePacket = _mMassivePacket;
                                    if (massivePacket == null || _mMassiveCount == 0)
                                        ThrowMalformedMassive("A massive packet fragment arrived without an active packet.");

                                    var fragmentSize = packet_size - 1;
                                    if (fragmentSize <= 0 || fragmentSize > _maxMassiveBytes - _mMassiveBytes)
                                        ThrowMalformedMassive("The massive packet payload exceeds the configured limit.");

                                    massivePacket!.WriteUInt8Array(packet_data.ReadBytes(fragmentSize));
                                    _mMassiveBytes += fragmentSize;
                                    _mMassiveCount--;
                                    if (_mMassiveCount == 0)
                                    {
                                ...
                                {
                                    ThrowMalformedMassive("The massive packet mode is invalid.");
                                }
                            }
                            else
                            {
        ...
        private void ThrowMalformedMassive(string reason)
        {
            ResetMassivePacket();
            Interlocked.Increment(ref _rejectedMassivePackets);
            throw new InvalidDataException($"[SecurityAPI::Recv] {reason}");
        }

        private void ResetMassivePacket()
        {
            _mMassiveCount = 0;
            _mMassiveBytes = 0;
            _mMassivePacket = null!;
