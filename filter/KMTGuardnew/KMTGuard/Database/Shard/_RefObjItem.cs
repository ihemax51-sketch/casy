...
namespace KMTGuard.Database.Shard
{
    public class _RefObjItem
    {
        ...
        public _RefObjCommon GetRefObjCommon
        {
            get
            {
                ...
                if (RefManager.RefObjCommons.TryGetValue(Link, out var cached))
                {
                    _refObjCommon = cached;
                    return cached;
                }

                throw new InvalidOperationException(
                    $"Reference object {Link} is missing from the preloaded cache.");
            }
        }
